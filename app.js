...
async function submitProject() {
    ...
    const { error } = await supabaseClient.from('Submissions').insert([{ student_name: currentUser.name, proposal_link: link, status: 'pending' }]);
...
async function postChallenge() {
    ...
    const { error } = await supabaseClient.from('Projects').insert([{ title, description, skills, company: currentUser.name }]);
    if (error) alert("Error: " + error.message);
    else { alert("Challenge Posted!"); document.getElementById('postModal').classList.add('hidden'); navigateTo('jobs'); }
}

async function postInsight() {
    const content = prompt("Write your industry insight:");
    if (content) {
        const { error } = await supabaseClient.from('Insights').insert([{ company: currentUser.name, content: content }]);
        if (error) alert("Error: " + error.message);
        else { alert("Insight Posted!"); navigateTo('drops'); }
    }
}
    ...
    if (page === 'home') {
        ...
        content.innerHTML = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="glass-card p-6 bg-white border"><h3 class="font-bold text-lg mb-4 text-slate-800">Upcoming Milestones</h3><div class="p-3 bg-gray-50 rounded-lg border">AI Proposal <span class="text-xs bg-amber-100 px-2 py-1 rounded">Due 2 days</span></div></div>
                <div class="glass-card p-6 bg-white border"><h3 class="font-bold text-lg mb-4 text-slate-800">Live Pulse</h3><div class="space-y-3 text-sm text-slate-600"><p>• Platform Active</p></div></div>
            </div>`;
    } 
    else if (page === 'jobs') {
        if (projectId) {
            const { data: p } = await supabaseClient.from('Projects').select('*').eq('id', projectId).single();
            content.innerHTML = `<button onclick="navigateTo('jobs')" class="mb-4 text-blue-600 font-bold">&larr; Back</button>
                <div class="bg-white p-8 rounded-2xl shadow-sm border">
                    ...
                    ${currentUser.role === 'student' ? `<button onclick="openModal()" class="w-full bg-blue-600 py-4 rounded-xl text-white font-bold">Submit Proposal</button>` : ''}
                </div>`;
        } else {
            content.innerHTML = `<div class="flex justify-between mb-6"><h2 class="text-2xl font-bold">Explore Challenges</h2></div>`;
            const { data: projects } = await supabaseClient.from('Projects').select('*');
            projects?.forEach(p => {
                content.innerHTML += `<div class="bg-white p-6 rounded-xl border mb-4 flex justify-between items-center">
                    <div><h3 class="font-bold text-xl">${p.title}</h3><p class="text-sm text-blue-600">${p.company}</p></div>
        ...
        content.innerHTML = `<div class="flex justify-between mb-8"><h2 class="text-3xl font-bold">Industry Insights</h2>${currentUser.role === 'industrialist' ? '<button onclick="postInsight()" class="bg-blue-600 text-white px-4 py-2 rounded-lg">+ Add Insight</button>' : ''}</div><div id="insightsContainer" class="space-y-4"></div>`;
        const { data: insights } = await supabaseClient.from('Insights').select('*');
        insights?.forEach(i => document.getElementById('insightsContainer').innerHTML += `<div class="bg-white p-6 rounded-2xl border"><h3 class="font-bold text-lg text-blue-600">${i.company}</h3><p class="mt-2">${i.content}</p></div>`);
    }
}
